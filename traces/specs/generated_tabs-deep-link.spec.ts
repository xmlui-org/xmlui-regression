...
test('tabs-deep-link', async ({ page }) => {
  ...
  await test.step('click: Tabs & deep links', async () => {
    await page.getByRole('link', { name: 'Tabs & deep links', exact: true }).click();
  });
  ...
  await test.step('click: Enable notifications', async () => {
    await page.waitForTimeout(1000);
    { const el = page.getByRole('checkbox', { name: 'Enable notifications', exact: true });
      await el.scrollIntoViewIfNeeded();
      await el.click(); }
    await expect(page.getByRole('checkbox', { name: 'Enable notifications' })).toBeChecked();
  });

  await test.step('click: Go to Overview', async () => {
    await page.getByRole('button', { name: 'Go to Overview', exact: true }).evaluate(node => {
      let el = node.parentElement;
      while (el && el !== document.documentElement) {
        if (el.scrollHeight > el.clientHeight) { el.scrollTop = 0; break; }
        el = el.parentElement;
  ...
  await test.step('click: Next tab', async () => {
    await page.getByRole('button', { name: 'Next tab', exact: true }).evaluate(node => {
      let el = node.parentElement;
      while (el && el !== document.documentElement) {
        if (el.scrollHeight > el.clientHeight) { el.scrollTop = 0; break; }
        el = el.parentElement;
    ...
    await page.getByRole('button', { name: 'Next tab', exact: true }).click();
  });

  await test.step('click: Previous tab', async () => {
    await page.getByRole('button', { name: 'Previous tab', exact: true }).evaluate(node => {
      let el = node.parentElement;
      while (el && el !== document.documentElement) {
        if (el.scrollHeight > el.clientHeight) { el.scrollTop = 0; break; }
        el = el.parentElement;
    ...
    await page.getByRole('button', { name: 'Previous tab', exact: true }).click();
  });
  } finally {
